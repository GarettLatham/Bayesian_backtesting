...
def main():
    try:
        ...
        cursor.execute("select Ticker, exchange from stocks where (exchangeDisplay='NASDAQ' OR exchangeDisplay='NYSE MKT' OR exchangeDisplay='NYSE' OR exchangeDisplay='OTC Market' OR exchangeDisplay='NASDAQ GIDS') AND (Type='S' OR Type='E') AND NOT (Ticker LIKE '%^%')")
    ...
    for symbol, exchange in sql_columns.tolist():
        ...
        try:
            X = yf.download(symbol,'1980-11-18','2021-10-07')
            ...
            #Close_groups = X.groupby('Close')
            # Apply the lambda function which will return -1.0 for down, 1.0 for up and 0.0 for no change.
            #Close_groups = Close_groups.transform(lambda x : np.sign(x.diff()))
            #X['direction'] = np.where(X['Close'].shift(-1) > X['Close'], "1", np.where(X['Close'].shift(-1) < X['Close'],"0", np.where(X['Close'].shift(-1) == X['Close'],"2", "3")))
            X['direction'] = np.where(X['Open'].shift(-1) > X['Close'], "1", np.where(X['Open'].shift(-1) < X['Close'],"0", np.where(X['Open'].shift(-1) == X['Close'],"2", "3")))
            #X['label'] = X['Close'].transform(lambda x : np.sign(x.diff()))
            #X['log_ret'] = np.log(X['Close']) - np.log(X['Close'].shift(-1))
            X['log_ret'] = (X['Open'].shift(-1) - X['Close']) / X['Close']
            #X['log_ret'] = np.where(X['label'] == 1, X['Close'].pct_change(), np.where(X['label'] == -1, (-1*X['Close'].pct_change()), 2))
            X['p0'] = np.where(X['Close'] > X['Close'].shift(1), 1, np.where(X['Close'] < X['Close'].shift(1),"0", np.where(X['Close'] == X['Close'].shift(1),"2", "3")))
            #X['p1'] = np.where(X['Close'].shift(1) > X['Close'].shift(2), 1, 0)
            #X['p2'] = np.where(X['Close'].shift(1) > X['Close'].shift(3), 1, 0)
            #X['p3'] = np.where(X['Close'].shift(1) > X['Close'].shift(4), 1, 0)
            ...
            with open("/home/user0/Desktop/laptop_backup/stocks_coo.csv", 'a') as f:
...
def ohlc_adj(dat):
    """
    :param dat: pandas DataFrame with stock data, including "Open", "High", "Low", "Close", and "Close", with "Close" containing adjusted closing prices
